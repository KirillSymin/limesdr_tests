...
// Write manual TXTSP correctors and un-bypass GC/PH
                              ...
                              bool have_phase, int phase)
{
    ...
    if (have_phase) {
        ...
        // IQCORR is 12-bit signed two's complement
        uint16_t iq12 = (uint16_t)((uint16_t)phase & 0x0FFF);
        rc |= LMS_WriteLMSReg(dev, 0x0203, iq12); // IQCORR
    }

    // Clear GC/PH bypass so manual values take effect
    uint16_t byp = 0;
    rc |= LMS_ReadLMSReg(dev, 0x0208, &byp);
    byp &= (uint16_t)~((1u<<0) | (1u<<1)); // clear PH_BYP (bit0) & GC_BYP (bit1)
...
int main(int argc, char** argv)
{
    ...
    bool SET_GI = false, SET_GQ = false, SET_PHASE = false;
    int  MAN_GI = 0,     MAN_GQ = 0,     MAN_PHASE = 0;
