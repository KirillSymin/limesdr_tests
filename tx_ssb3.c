// tx_sine_nco_corr.c
// Generate an RF sine using LimeSDR's NCO (TX DC -> NCO mix).
// - Calibration is OFF by default (--calibrate on to enable).
// - Always prints: sample rates, LO, gain, LPF BW, NCO settings, effective RF tone,
//   and TXTSP correctors: Gain (GCORRI/GCORRQ), Phase (IQCORR), DC (DCCORRI/DCCORRQ) + bypass flags.
...
// ---------- register-level helpers for TXTSP correctors ----------
...
static void print_tx_correctors(lms_device_t* dev, int ch /*0=A,1=B*/){
    ...
    (void)LMS_ReadLMSReg(dev, 0x0208, &reg_byp); // PH/GC/DC BYP flags
...
int main(int argc, char** argv)
{
    ...
    // Always print correctors + current state (post calibration or not)
    ...
    // Constant I/Q (DC) -> hardware NCO creates RF sine at LO ± NCO
...
cleanup:
    if (txs.handle){
        // short zero burst for a clean stop
